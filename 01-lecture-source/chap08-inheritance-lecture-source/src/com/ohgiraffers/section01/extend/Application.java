...
public class Application {
    public static void main(String[] args) {
        ...
        * 상속은 현실세계상속과 비슷한 개념이다.
        * 부모가 가지고 있는 재산 (자바에서는 클래스가 가지는 멤버)를 자식이 물려받는의미이다.
        * 클래스 또한 부모 클래스와 자식클래스로 역할을 나누어서 부모가 가지는 멤버를 지식이 물려받아
        * 자기의 멤버인 거처럼 사용할 수 있도록 만든 기술이다.
        *
        * 단순 물려받는 개념보다 조금 더 나아간다면 자바에서의 상속은
        * 부모클래스의 확장(extend)의 개념을 가진다.
        * 물려 받아서 자신의 것처럼 사용하는 것 뿐만 아니라 추가적인 멤버도 작성이가능하다.
        * 특히 메소드 재정의(Overriding )라는 기술을 이용해서 부모가 가진 메소드를 재정의하는 것도 가능하다.
        * */

        /*
        * 메소드 재정의(oveeriding)이란 ? 부모가 가지는 메소드 선언부를 그대로 사용하면서 자식 클래스가
        ...
        * */
        ...
        /*
        * 이러한 상속이라는 기술을 사용하게 되면 믿게 되는 이점은 두가지가 있다.,
        * 1. 새로운 클래스를 작성시 기존의 작성한 클래스를 재시용할 수 있다.
        * 1-1 재사용 시 생산성을 크게 향상시킬 수 있다,(새롭게 작성하는 거보다 빠르다)
        * 1-2 공통적으로 사용하는 코드가 부모클래스에 존재하면 수정사항이 생길 시 부모 클래스만 수정해도 전체적으로
        * 적용된다 ( 유지 보수성 증가 )
        * 2. 클래스간의 계층 관계까 형성되며 다형성이 문법적인 토대가 된다.
        *
        * 상속으로 인한 단점
        * 1. 부모클래스의 기능을 추가/변경시에 자식클래스가 정상 동작하는지 예측이 힘들다.
        *   상속 구조가 복잡해질 수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이 증가한다는 장점과는 반대로
        *   유지 보수에 악영향을 미친다.
        ...
        * */
        /*
        * 장점과 단점을 고려했을 때 상속은 재사용이라는 장점만을 바라보게 되면 오용의 가능성이 있기 때문에
