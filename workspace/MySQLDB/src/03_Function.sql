...
--rount(숫자, 자리수) - 소수점 자리수만큼 남기고 아래서 반올림
...
--DATE_SUB(날짜,INTERVAL 뻘 숫자 기준값) - 날짜에서 기준값 단위로 뺄 숫자만큼 뺀다.

--YEAR(날짜) - 날짜의 연도 추출
-- MONTH(날짜) - 날짜의 월 추출
-- MONTHNAME(날짜) - 날짜의 월을 영어로 추출
-- DAYOFMONTH(날짜) - 날짜의 월별 일자 추춘
...
-- %T 시간(24시)를 hh:mm:ss 형식으로 

-- 집계 함수(그룹함수)
-- COUNT(필드명) - NULL 값이 아닌 래코드 수를 구한다
