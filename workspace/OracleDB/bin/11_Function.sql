...
--이어붙이기 
select concat('하이미디어IT', '아카데미'), '하이미디어 IT'|| '아카데미' from dual;
select concat(empno, ename) from emp; 

-- substr(): 문자열 추출(데이터, 인덱스(1), 카운트)
select substr('홍길동 만세', 2,4) from dual;-- 길동 만 
-- substr 의 경우 자바의 substring 처럼 시작번째부터 끝번쨰 +1 이 아니라
-- 시작 번째 부터 글자수를 나타 냅니다. 위의 예경우 2번째 글자부터 4글자 표시


-- 날짜 처리 관련함수
-- sysdate: 날짜
select sysdate from dual;
-- insert  명령 그리고 그 외에 오늘 날짜가 필요한 곳에 폭 넓게 사용됩니다
...
-- YYYY -> 년도 표시
select to_char(sysdate, 'yy"년"') form dual; --> 24년
select to_char(sysdate, 'y"년"') form dual; -->4년
