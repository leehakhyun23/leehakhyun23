...
-- 가상이란 단어는 실직적으로 데이터를 저장하고 있지 않기 때문에 붙인 것이고,
-- 테이블이란 단어는 실질적으로 데이터를 저장하고 있지 않더라도 사용자는 마치
-- 테이블을 사용하는 것과 동일하게 뷰를 사요할 수 있기 때문에 붙인것 

-- 뷰는 기본테이블에서 파생된 객체로서 기본테이블에 대한 나의 쿼리문임.
-- 실제 테이블에 저장된 데이터를 뷰를 통해서 볼 수 있도록함 .
...
select a.rentdate as 대여일자 , a.bnum as 대여도서번호 , b.title as 제목 ,  a.mnum as 대여회원번호 , c.name as 이름 , b.rentprice-a.discount as 매출액 
